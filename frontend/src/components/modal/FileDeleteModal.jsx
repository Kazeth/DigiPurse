...
function FileDeleteModal({ fileName, fileSize, onClose, actor, onSuccess }) {
    const [isDeleting, setIsDeleting] = useState(false);

    async function handleFileDelete() {
        setIsDeleting(true);
        try {
            const success = await actor.deleteFile(fileName);
            if (success) {
                onSuccess?.();
                onClose();
            } else {
            ...
            console.error('Delete process terminated', error);
        } finally {
            setIsDeleting(false);
        }
    }
